...
function buildLatestDailyPerformanceRows(rows: GoogleOsRow[]) {
  const groups = new Map<string, GoogleOsRow[]>();
  ...
  const ascending = Array.from(groups.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([date, dateRows]) => {
      const a = aggregate(dateRows);
  ...
  const withDelta = ascending.map((row, index) => {
    ...
    return {
      ...row,
      spendDelta: safeDiv(row.spend - previous.spend, previous.spend),
      revenueDelta: safeDiv(row.revenue - previous.revenue, previous.revenue),
      roasDelta: safeDiv(row.roas - previous.roas, previous.roas),
      purchaseDelta: safeDiv(row.purchases - previous.purchases, previous.purchases),
      cpaDelta: safeDiv(row.cpa - previous.cpa, previous.cpa),
    };
  });

  return withDelta.slice(-30).reverse();
}
...
function Movement({
  ...
  if (!Number.isFinite(value) || value === 0) {
    return <small className="gos-daily-move neutral">—</small>;
  }
  ...
  return (
    <small className={`gos-daily-move ${good ? "green" : "red"}`}>
      {sign}{pct(value)}
    </small>
  );
}

function DailyTableCell({
  value,
  move,
  inverse = false,
  children,
  ...
  return (
    <span className="gos-daily-cell">
      <strong>{value}</strong>
      {typeof move === "number" ? <Movement value={move} inverse={inverse} /> : null}
      {children}
    </span>
  );
}
...
export function CampaignsTab({ model }: { model: GoogleOsModel }) {
  ...
  const latestDailyRows = useMemo(() => {
    return buildLatestDailyPerformanceRows(model.rows);
  }, [model.rows]);
  ...
  return (
    <section className="gos-page all-campaigns-summary-page">
      <div className="gos-panel gos-campaign-accordion-panel">
        ...
        <div className="gos-daily-performance-table">
          <div className="gos-daily-performance-head">
            <div>
          ...
          <div className="gos-daily-table">
            <div className="gos-daily-table-header">
              <span>Date</span>
              <span>Spend</span>
              <span>Revenue</span>
              <span>ROAS</span>
            ...
            {latestDailyRows.map((row) => (
              <div key={row.date} className="gos-daily-table-row">
                <span className="gos-daily-date">{formatGoogleOsDateLabel(row.date)}</span>

                <DailyTableCell value={compactMoney(row.spend)} move={row.spendDelta} />

                <DailyTableCell value={compactMoney(row.revenue)} move={row.revenueDelta} />

                <DailyTableCell value={x(row.roas)} move={row.roasDelta} />

                <DailyTableCell value={row.purchases.toFixed(0)} move={row.purchaseDelta} />

                <DailyTableCell value={compactMoney(row.cpa)} move={row.cpaDelta} inverse />

                <span>{pct(row.ctr)}</span>
                <span>{pct(row.cvr)}</span>
                <span>{compactMoney(row.impressions)}</span>
                <span>{compactMoney(row.clicks)}</span>
              </div>
            ))}
          </div>
        </div>
